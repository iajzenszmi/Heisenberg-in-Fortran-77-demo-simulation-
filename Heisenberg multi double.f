...
~/downloads $ cat heisen_multi_double.f
      PROGRAM HEISENM
C
...
C
      ...
      DOUBLE PRECISION SIGMAS(NCASE), K0S(NCASE), X0S(NCASE)
      INTEGER IC

      DATA SIGMAS /0.5D0, 1.0D0, 2.0D0, 1.0D0/
      DATA K0S    /0.0D0, 0.0D0, 0.0D0, 2.0D0/
      DATA X0S    /0.0D0, 0.0D0, 0.0D0, 0.0D0/
  ...
  100 CONTINUE
      ...
      DOUBLE PRECISION SIGMA, K0, X0
      DOUBLE PRECISION PI, HBAR, L, DX, DK, EPS
      DOUBLE PRECISION X(512), P(512)
      DOUBLE PRECISION REPSI(512), IMPSI(512)
      DOUBLE PRECISION REPHI(512), IMPHI(512)

      DOUBLE PRECISION AMP, EDGE
      DOUBLE PRECISION PROBX, PROBK
      DOUBLE PRECISION NORMX, NORMK
      DOUBLE PRECISION XMEAN, X2MEAN, VARX, DELX

      DOUBLE PRECISION PMFFT, P2FFT, VARFFT, DELPFFT
      DOUBLE PRECISION PMFD,  P2FD,  VARFD,  DELPFD

      DOUBLE PRECISION KVAL, THETA, CTH, STH
      DOUBLE PRECISION SUMRE, SUMIM
      DOUBLE PRECISION DARE, DAIM, D2ARE, D2AIM

      DOUBLE PRECISION EXX, EXPM, EXDP, EXPROD
      DOUBLE PRECISION ERRX, ERRPMF, ERRPFF
      DOUBLE PRECISION ERRDPF, ERRDPD, ERRUF, ERRUD
...
C     X MOMENTS
  ...
  130 CONTINUE
      ...
      IF (VARX .LT. 0.0D0 .AND. ABS(VARX) .LT. EPS) VARX = 0.0D0
      IF (VARX .LT. 0.0D0) THEN
...
C     FOURIER MOMENTUM CALCULATION
  ...
  180    CONTINUE
         ...
         REPHI(J) = SUMRE / SQRT(2.0D0*PI)
         IMPHI(J) = SUMIM / SQRT(2.0D0*PI)
  200 CONTINUE
  ...
  230 CONTINUE
      ...
      IF (VARFFT .LT. 0.0D0 .AND. ABS(VARFFT) .LT. EPS)
     1    VARFFT = 0.0D0
      IF (VARFFT .LT. 0.0D0) THEN
...
C     <P2> = -HBAR**2 * INTEGRAL (A*D2A/DX2 + B*D2B/DX2) DX
      ...
      P2FD = 0.0D0
      DO 300 I = 2, N-1
         DARE = (REPSI(I+1) - REPSI(I-1)) / (2.0D0*DX)
         DAIM = (IMPSI(I+1) - IMPSI(I-1)) / (2.0D0*DX)

         D2ARE = (REPSI(I+1) - 2.0D0*REPSI(I)
     1          + REPSI(I-1)) / (DX*DX)
         D2AIM = (IMPSI(I+1) - 2.0D0*IMPSI(I)
     1          + IMPSI(I-1)) / (DX*DX)
  ...
  300 CONTINUE
      ...
      IF (VARFD .LT. 0.0D0 .AND. ABS(VARFD) .LT. EPS)
     1    VARFD = 0.0D0
      IF (VARFD .LT. 0.0D0) THEN
